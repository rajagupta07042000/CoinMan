...
public class CoinMan extends ApplicationAdapter {
	...
	Texture background;
	Texture[] man;
	int manState=0;//for man to be running
	int pause=0;//to make man run slow
	float gravity=0.2f;
	float velocity=0;
	int manY=0;
	...
	@Override
	//render is called again and again in the game
	public void render () {
		batch.begin();
		batch.draw(background,0,0,Gdx.graphics.getWidth(),Gdx.graphics.getHeight());//how bg is to be displayed
		...
		if(gameState==1)
		{
			...
			if(Gdx.input.justTouched())//ato get him go up after touching bootom on click
			{
				velocity=-10;
			}
			...
			if(manY<0)//not to go under bottom
			{
				manY=0;
			}
